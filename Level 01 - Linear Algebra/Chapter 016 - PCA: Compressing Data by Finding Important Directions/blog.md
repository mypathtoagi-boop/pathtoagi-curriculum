...
Chapter 006 taught us that coordinates depend on basis.

Chapter 006 taught us that some directions may be redundant.
...
PCA asks the same question SVD asked, but now the object is a **cloud of data** rather than an arbitrary transformation: which directions contain most of the structure?

Imagine rotating the coordinate axes until one axis lies along the long direction of the cloud. If almost all variation lies there, then every two-dimensional point can be represented fairly well by just one coordinate along that axis. Compression is therefore a change of basis followed by deliberately discarding weak directions.

Centering matters because PCA studies variation **around the mean**. Scaling matters because variance carries units: a feature measured in thousands can dominate one measured in single digits even if it is not conceptually more important.

> 🎛️ First use **Step zero: centre it**. Then hunt for the maximum in **Variance along a line**. Finally turn the direction away in **Project and rebuild** and watch exactly what information the discarded component was carrying.
...
$$
\boldsymbol{\mu} = \frac1n\sum_i\mathbf{x}_i.
$$
...
$$
\|X_c\mathbf{v}\|^2 = \mathbf{v}^TX_c^TX_c\mathbf{v}.
$$

Ignoring the constant factor $1/(n-1)$,

$$
\text{variance along }\mathbf{v} = \mathbf{v}^TC\mathbf{v}.
$$
...
This is Chapter 006's change of basis, now chosen from data.
...
$$
\boxed{ \text{explained variance ratio}_i = \frac{\lambda_i}{\sum_j\lambda_j} }
$$
