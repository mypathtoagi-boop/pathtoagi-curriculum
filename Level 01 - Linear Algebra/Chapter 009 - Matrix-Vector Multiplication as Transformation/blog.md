...
Stop thinking of a matrix as a table for a moment. Think of graph paper with two basis arrows attached to it.

A 2×2 matrix tells you where those two basis arrows land. Once their destinations are fixed, every other point has no freedom left: its coordinates are the same recipe, now built from the transformed basis. That is why the **columns** of a matrix are so important — they are literally the images of the basis vectors.

This single picture explains scaling, reflection, shear and rotation without memorizing four unrelated formulas. It also explains linearity: straight grid lines remain straight, parallel lines remain parallel, and the origin stays fixed.

> 🎛️ Spend time in **The columns** before moving to the named transformations. Drag one column at a time and predict what the entire grid will do. When you can read a matrix as motion, the later algebra becomes much easier.
...
$$
\boxed{ W\mathbf{x} = \begin{bmatrix}7\\4\end{bmatrix} }
$$
...
$$
W\mathbf{e}_1 = \begin{bmatrix}2\\-1\end{bmatrix} =\mathbf{c}_1
$$

and

$$
W\mathbf{e}_2 = \begin{bmatrix}1\\2\end{bmatrix} =\mathbf{c}_2.
$$
...
Rather than take that on faith, move the basis yourself. Drag either arrowhead
below: the grid is carried along with it, and the matrix beside it is nothing
more than the two arrows' coordinates written down in two columns.

```sim
kind: grid-morph
v1: [2, -1]
v2: [1, 2]
caption: The dashed arrows are where the basis started. Drag the solid ones anywhere.
```
...
$$
S\begin{bmatrix}x\\y\end{bmatrix} = \begin{bmatrix}2x\\3y\end{bmatrix}.
$$
...
$$
H\begin{bmatrix}x\\y\end{bmatrix} = \begin{bmatrix}x+y\\y\end{bmatrix}.
$$
